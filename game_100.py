...
def run():
	...
	#question base
	qs = ["Koliko sreća grabi onaj tko rano rani?",
			"Koji je predmet vrijedan 1000 riječi?",
			"Što ne pada daleko od stabla?",
			"Bez čega nema nauke?",]

	#wrong answers base
	wanss = [["1","3","4"],
			["rječnik","enciklopedija","knjiga"],
			["naranča","kruška","šljiva"],
			["bez rada","bez suza","bez boli"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_100_label.destroy()
		
			money_100_button = Button(money_100_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_100_button.config(width=24,height=1)
			money_100_button.bind("<Button>",next_200)
			money_100_button.pack()
	...
	def next_200(event):
		money_100_button.bind("<Button>",next_200_false)
		game_200.run()

	def next_200_false(event):
		pass
