...
def run():
	...
	#question base
	qs = ["Izbaci uljeza!",
			"Koja se godina uzima kao početak 1. svjetskog rata?",
			"Autor kojeg naslova je William Shakespeare?",
			"Broj pi iznosi približno...",]

	#wrong answers base
	wanss = [["W. A. Mozart","L. v. Beethoven","J. S. Bach"],
			["1918.","1939.","1991."],
			["Antigona","Odiseja","Eneida"],
			["5","2,72","bezbroj"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_1000_label.destroy()
		
			money_1000_button = Button(money_1000_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_1000_button.config(width=24,height=1)
			money_1000_button.bind("<Button>",next_2000)
			money_1000_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 500 KN")
		money_won = "500"
	...
	def next_2000(event):
		money_1000_button.bind("<Button>",next_2000_false)
		game_2000.run()

	def next_2000_false(event):
		pass
	...
	#game system
	money_1000_label.config(fg="red")
	jok_50_used = False
