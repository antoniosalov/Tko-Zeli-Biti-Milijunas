...
def run():
	...
	#question base
	qs = ["Kako se zvao pas Adolfa Hitlera?",
			"Koje je srednje ime princa Georgea od Cambridgea?",
			"Iz koje države dolazi medvjedić Paddington?",
			"Tko je bio prvi čovijek koji je išao u svemir dva puta?",]

	#wrong answers base
	wanss = [["Barry","Fido","Pickles"],
			["Louis","Arthur","Charles"],
			["Iz Engleske","Iz Kanade","Iz Meksika"],
			["Vladimir Titov","Michael Collins","Yuri Gagarin"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			q_label.config(text="VAŠ ODGOVOR JE TOČAN. OSVOJILI STE 1000000 KN. VI STE MILIJUNAŠ !!!")
			money_won = "1000000"
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 500000 KN")
		money_won = "500000"
	...
	#game system
	money_1000000_label.config(fg="red")
	jok_50_used = False
