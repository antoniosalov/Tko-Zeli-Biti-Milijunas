...
def run():
	...
	#question base
	qs = ["S kime je Freddie Mercury trebao pjevati pjesmu 'Barcelona' na otvaranju OI?",
			"Koja je naša pjevačica oborila rekord za 'najviši glas' Eurovizije?",
			"Koji je hrvatski naziv za sonet?",
			"Kome se na pola životnog puta noga u mračnoj šumi stvori?",]

	#wrong answers base
	wanss = [["Maira Callas","Montserrat Marti","Jessye Norman"],
			["Daniela Martinović","Doris Dragović","Vesna Pisarović"],
			["rimalica","pjevalica","trubalica"],
			["Shakespeare","Petrarca","Boccaccio"],]

	#correct answers base
	canss = ["Montserrat Cabale",
			"Maja Blagdan",
			"zvonjelica",
			"Dante",]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_125000_label.destroy()
		
			money_125000_button = Button(money_125000_frame,text="DALJE !!",bg="lightblue",fg="red",font="arial 10 bold")
			money_125000_button.config(width=24,height=1)
			money_125000_button.bind("<Button>",next_250000)
			money_125000_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 64000 KN")
		money_won = "64000"
	...
	def next_250000(event):
		money_125000_button.bind("<Button>",next_250000_false)
		game_250000.run()

	def next_250000_false(event):
		pass
	...
	#game system
	money_125000_label.config(fg="red")
	jok_50_used = False
