...
def run():
	...
	#question base
	qs = ["DNK je građena od pentoze deoksibiroze, dušične baze(A,C,G,T) i kakve skupine?",
			"Što se nalazi u unutrašnjosti mitohondrija?",
			"Periodni sustav elemenata sastoji se od...",
			"Koliko vrsta vitamina postoji?",]

	#wrong answers base
	wanss = [["dušične","ugljične","sumporne"],
			["citoplazma","šećer","voda"],
			["8 per. i 12 sku.","9 per. i 18 sku.","10 per. i 13 sku."],
			["6","9","17"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_16000_label.destroy()
		
			money_16000_button = Button(money_16000_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_16000_button.config(width=24,height=1)
			money_16000_button.bind("<Button>",next_32000)
			money_16000_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 8000 KN")
		money_won = "8000"
	...
	def next_32000(event):
		money_16000_button.bind("<Button>",next_32000_false)
		game_32000.run()

	def next_32000_false(event):
		pass
	...
	#game system
	money_16000_label.config(fg="red")
	jok_50_used = False
