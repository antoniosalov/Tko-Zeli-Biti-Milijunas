...
def run():
	...
	#question base
	qs = ["Kakve su noge lažljiva čovijeka?",
			"Za koliko je dana, dosta svakog gosta?",
			"Što ne čini čovijeka?",
			"Kakva vrata otvara lijepa riječ?",]

	#wrong answers base
	wanss = [["debele","tanke","duge"],
			["5","7","10"],
			["jakna","srce","duša"],
			["kamena","teška","debela"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_200_label.destroy()
		
			money_200_button = Button(money_200_frame,text="DALJE !!",bg="lightblue",fg="red",font="arial 10 bold")
			money_200_button.config(width=24,height=1)
			money_200_button.bind("<Button>",next_300)
			money_200_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 100 KN")
		money_won = "100"
	...
	def next_300(event):
		money_200_button.bind("<Button>",next_300_false)
		game_300.run()

	def next_300_false(event):
		pass
