...
def run():
	...
	#question base
	qs = ["Kakva je pjesma balada?",
			"Koga nazivamo ocem hrvatske književnosti?",
			"Tko je glavni junak Ilijade?",
			"Što je po nacionalnosti bio E. A. Poe",]

	#wrong answers base
	wanss = [["epsko-dramska","dramsko-lirska","diskurzivna"],
			["August Šenoa","Tin Ujević","Petar Zoranić"],
			["Patroklo","Agamemnon","Hektor"],
			["francuz","englez","njemac"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_2000_label.destroy()
		
			money_2000_button = Button(money_2000_frame,text="DALJE !!",bg="lightblue",fg="red",font="arial 10 bold")
			money_2000_button.config(width=24,height=1)
			money_2000_button.bind("<Button>",next_4000)
			money_2000_button.pack()
	...
	def next_4000(event):
		money_2000_button.bind("<Button>",next_4000_false)
		game_4000.run()

	def next_4000_false(event):
		pass
	...
	#game system
	money_2000_label.config(fg="red")
	jok_50_used = False
