...
def run():
	...
	#question base
	qs = ["Kakvu je medalju osvojila Hrvatska nogometna repreznetacija na SP-2018?",
			"Koliko krugova ima logo OI?",
			"Koliko rupa ima biljarski stol?",
			"Koliko najviše žutih kartona može dobiti 1 igrač u 1 nogometnoj utakmici?",]

	#wrong answers base
	wanss = [["zlatnu","brončanu","cinčanu"],
			["3","7","10"],
			["2","4","8"],
			["1","3","5"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_300_label.destroy()
		
			money_300_button = Button(money_300_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_300_button.config(width=24,height=1)
			money_300_button.bind("<Button>",next_500)
			money_300_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 200 KN")
		money_won = "200"
	...
	def next_500(event):
		money_300_button.bind("<Button>",next_500_false)
		game_500.run()

	def next_500_false(event):
		pass
