...
def run():
	...
	#question base
	qs = ["Koja je visina planine Kilimanjro?",
			"Koje je površina otoka Grenlanda?",
			"Koje se godine odgirala bitka s Tatarima na Grobničkom polju?",
			"S koliko je godina umro Napoleon Bonaparte?",]

	#wrong answers base
	wanss = [["6295 m","5695 m","4895 m"],
			["1381231 km2","2737426 km2","684317 km2"],
			["1424 g.","1444 g.","1224 g."],
			["37","49","63"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_32000_label.destroy()
		
			money_32000_button = Button(money_32000_frame,text="DALJE !!",bg="lightblue",fg="red",font="arial 10 bold")
			money_32000_button.config(width=24,height=1)
			money_32000_button.bind("<Button>",next_64000)
			money_32000_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 16000 KN")
		money_won = "16000"
	...
	def next_64000(event):
		money_32000_button.bind("<Button>",next_64000_false)
		game_64000.run()

	def next_64000_false(event):
		pass
	...
	#game system
	money_32000_label.config(fg="red")
	jok_50_used = False
