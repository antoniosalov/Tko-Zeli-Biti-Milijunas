...
def run():
	...
	#question base
	qs = ["Od koliko se 'malih kockica' sastoji obična rubikova kocka",
			"Komplementarni se kutovi međusobno nadopunjavaju do...",
			"Kojom računskom operacijom on minuenda i suptrahenda dobijemo diferenciju?",
			"Kojeg su podrijetla ovi prefiksi za mjerne jedinice (deci,centi,mili)?",]

	#wrong answers base
	wanss = [["9","35","54"],
			["180 stupnjeva","270 stupnjeva","360 stupnjeva"],
			["zbrajanjem","množenjem","djeljenjem"],
			["latinskog","francuskog","arapskog"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_4000_label.destroy()
		
			money_4000_button = Button(money_4000_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_4000_button.config(width=24,height=1)
			money_4000_button.bind("<Button>",next_8000)
			money_4000_button.pack()
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 2000 KN")
		money_won = "2000"
	...
	def next_8000(event):
		money_4000_button.bind("<Button>",next_8000_false)
		game_8000.run()

	def next_8000_false(event):
		pass
	...
	#game system
	money_4000_label.config(fg="red")
	jok_50_used = False
