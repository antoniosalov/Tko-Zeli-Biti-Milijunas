...
def run():
	...
	#question base
	qs = ["Tko je dobitnica nagrade hrvatskog glumišta za najbolju glavnu žensku ulogu 2019?",
			"Koliko je filmova snimio Sean Connery kao James Bond?",
			"Tko glumi Stuarta u 'Teoriji velikog praska'?",
			"Koji film nije režirao James Cameroon?",]

	#wrong answers base
	wanss = [["Mila Elegović","Zrinka Cvitešić","Ana Veselčić"],
			["3","5","8"],
			["Simon Helberg","Johnny Galecki","Wil Wheaton"],
			["Titanic","Avatar","Solaris"],]

	#correct answers base
	canss = ["Olga Pakalović",
			"7",
			"Kevin Sussman",
			"Signs",]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_64000_label.destroy()
		
			money_64000_button = Button(money_64000_frame,text="DALJE !!",bg="black",fg="red",font="arial 10 bold")
			money_64000_button.config(width=24,height=1)
			money_64000_button.bind("<Button>",next_125000)
			money_64000_button.pack()
	...
	def next_125000(event):
		money_64000_button.bind("<Button>",next_125000_false)
		game_125000.run()

	def next_125000_false(event):
		pass
	...
	#game system
	money_64000_label.config(fg="red")
	jok_50_used = False
