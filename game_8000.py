...
def run():
	...
	#question base
	qs = ["Na poleđini koje novčanice se nalazi Zgrada glavnog zapovjedništva u Osjeku?",
			"Čiji se kip nalazi na poleđini novčanice od 1000kn?",
			"Koji se grb ne nalazi u kruni hrvatskog grba?",
			"Što predstavlja drugi slijeva grb u kruni hrvatskog grba?",]

	#wrong answers base
	wanss = [["50kn","100kn","500kn"],
			["bana Jelačića","kralja Zvonimira","Ante Starčevića"],
			["grb Istre","grb Dalmacije","grb Slavonije"],
			["Istru","Dalmaciju","grad Zagreb"],]
	...
	def ansA_func(event):
		...
		if ansA_button.cget("fg") == "green":
			...
			money_8000_label.destroy()
		
			money_8000_button = Button(money_8000_frame,text="DALJE !!",bg="lightblue",fg="red",font="arial 10 bold")
			money_8000_button.config(width=24,height=1)
			money_8000_button.bind("<Button>",next_16000)
			money_8000_button.pack()

		else:
			q_label.config(text="VAŠ ODGOVOR JE NETOČAN. OSVOJILI STE 1000 KN")
			money_won = "1000"
	...
	def give_up_func(event):
		...
		q_label.config(text="ODUSTALI STE. OSVOJILI STE 4000 KN")
		money_won = "4000"
	...
	def next_16000(event):
		money_8000_button.bind("<Button>",next_16000_false)
		game_16000.run()

	def next_16000_false(event):
		pass
	...
	#game system
	money_8000_label.config(fg="red")
	jok_50_used = False
